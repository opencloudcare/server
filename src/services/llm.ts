...
export const askModel = async (contents: any, model: string, searchWeb = false, files: {mimeType: string, data: string}[]) => {
  const systemTurn = {
    role: "user",
  ...
  if (searchWeb) {
    ...
    if (searchQuery) {
      ...
      // TODO: save websearch and results for future searches
