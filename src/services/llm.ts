...
export const askModel = async (contents: any, searchWeb=false) => {
  ...
  let searchTurn = null
  if (searchWeb) {
    const lastMessage = contents[contents.length - 1].content
    const searchResponse = await tvly.search(lastMessage, {maxResults: 5})
    const searchContext = searchResponse.results
      .map((r: any) => `**${r.title}**\n${r.content}\nSource: ${r.url}`)
      .join("\n\n---\n\n")
    console.log("ASKING THE WEB: ", lastMessage)
  ...
  return await ai.models.generateContentStream({
    ...
    contents: [systemTurn, ...(searchTurn ? [searchTurn] : []), ...contents.map((m: any) => ({
      role: m.role,
      parts: [
        {text: m.content},
